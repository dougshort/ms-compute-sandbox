...
function SNMPSTAT
{
sudo /bin/systemctl -l status  snmpd.service
echo ""
echo "Process List:"
...
function SNMPSTART
{
sudo /bin/systemctl stop snmpd.service
sudo /bin/systemctl start snmpd.service
sudo /bin/systemctl -l status  snmpd.service
echo ""
}
...
/bin/rpm -qf /etc/redhat-release
echo ""
sudo /usr/sbin/dmidecode --type 1
cat /proc/meminfo | grep MemT
cat /proc/meminfo|grep -i swap
sudo /usr/sbin/dmidecode -t 4|grep -E '\''Intel|Core|Version'\'|grep -v Unknown|grep -v Manuf
echo ""
echo "NETWORK:"
/sbin/ip a
}
